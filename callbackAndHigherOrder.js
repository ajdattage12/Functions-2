...
// CODE HERE
const multipliedNumbersCB = (num1, num2) => {
  return num1 * num2;
};
...
const multiply = (num1, num2, callback) => {
  if(+num1 && +num2){
      ...
      console.log("You need to send me numbers.");
  };
};
const answer = multiply (4, 3, multipliedNumbersCB);
...
const firstNameCB = (names[0])

const first = (arr, firstNameCB) => {
  return (arr, firstNameCB);
}

first([names], firstNameCB)
console.log("The first name is " + firstNameCB);
// UNCOMMENT THE FUNCTION CALL BELOW
// RUN THIS FILE WITH NODE
// CHECK YOUR ANSWER
...
const lastNameCB = (names[names.length-1])

const last = (arr, lastNameCB) => {
  return (arr, lastNameCB);
}

last([names], lastNameCB)
console.log("The first name is " + lastNameCB);
...
const uniq = (arr, callback) => {
  let uniqArr = arr.filter((name, index) => arr.indexOf(name) === index)
  callback(uniqArr)
}
...
uniq (names, unique => {
  console.log(`The new names array with all of the duplicate names removed is ${unique}`)
})
